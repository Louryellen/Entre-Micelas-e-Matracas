...
export function initCena1() {
  ...
  const subtitulo =
    document.getElementById("subtituloCena") ||
    document.querySelector("header .subtitle");

  const SUBTITULO_PADRAO = subtitulo?.textContent || "";
  const setSubtitulo = (t) => {
    if (subtitulo) subtitulo.textContent = t;
  };
  ...
  btnIrInvestigacao?.addEventListener("click", () => {
    if (btnIrInvestigacao.disabled) return;
    tocarSequenciaDeVideos();
  });
  ...
  reagentes.addEventListener("click", () =>
    openInfo({
      ...
      general:
        "Soluções químicas preparadas para reagir com substâncias presentes na água.",
  ...
  residuos.addEventListener("click", () =>
    openInfo({
      ...
      general:
        "Conjunto de frascos que guardam soluções já utilizadas ou sobras de experimentos anteriores, organizados na bancada do laboratório.",
  ...
  micro.addEventListener("click", () =>
    openInfo({
      ...
      general:
        "Equipamento elétrico utilizado em alguns protocolos para aquecer soluções, vidrarias ou materiais, e que costuma ficar instalado em um ponto fixo do laboratório.",
  ...
  cafe.addEventListener("click", () =>
    openInfo({
      ...
      general:
        "Caneca pessoal que costuma acompanhar a pesquisadora nas pausas entre uma etapa e outra do trabalho experimental.",
  ...
  plantas.addEventListener("click", () =>
    openInfo({
      ...
      general:
        "Vasos decorativos que deixam o ambiente de laboratório mais agradável, trazendo um pouco de verde para perto dos equipamentos.",
...
export function initCena2() {
  ...
  const INFO_ERROS = {
    erro1: {
      ...
      impact:
        "A película reduz a troca gasosa com o ar, pode intoxicar organismos aquáticos e agrava o mau cheiro e a degradação da qualidade da água.",
      acao:
        "Ação: conter/recolher o óleo (barreiras/absorventes), orientar a comunidade sobre descarte correto e acionar coleta/ponto de entrega (logística reversa).",
    },
    erro2: {
    ...
    erro4: {
      ...
      impact:
        "Pode causar dermatites, gastroenterites e outras doenças de veiculação hídrica.",
    ...
    erro5: {
      ...
      impact:
        "Gera microplásticos, dano à fauna e liberação de contaminantes ao longo do tempo.",
    ...
    erro6: {
      ...
      impact:
        "Contamina solo e água ao longo do tempo, com risco crônico para a biota e humanos.",
  ...
  const MAP_ERROS = {
    erro1: { x: 0.125, y: 0.47, w: 0.235, h: 0.24 },
